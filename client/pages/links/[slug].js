...
const Links = ({ query, category, links, totalLinks, linksLimit, linkSkip }) => {
     ...
     const head = () => (
        <Head>
            ...
            <meta name="discription" content={stripHTML(category.content.substring(0, 160))}/>
     ...
     const handelClick = async (linkId) => {
         const response = await axios.put(`${API}/click-count`, {linkId})
    ...
    popular.map((l, i) => (
        <div key={i} className="row alert alert-secondary p-2">
            ...
            <div className="col-md-4 pt-2">
                ...
                {new Date(l.createdAt).toLocaleString()} by {l.postedBy.name}
                </span>
            </div>
    ...
    const listOfLinks = () => (
        allLinks.map((l, i) => (
            <div key = {i} className="row alert alert-primary p-2">
                <div className="col-md-8" onClick={e => handelClick(l._id)}>
                    <a href={l.url} target="_blank">
                        <h5 className="pt-2">{l.title}</h5>
                        <h6 className="pt-2 text-danger" style={{fontSize: '12px'}}>
                            {l.url}
    ...
    const loadMoreButton = () => {
        return (
            size > 0 && size >= limit && (
                <button onClick={loadMore} className="btn btn-outlinr-primary btn-lg">Load More</button>
            )
        )
    }
